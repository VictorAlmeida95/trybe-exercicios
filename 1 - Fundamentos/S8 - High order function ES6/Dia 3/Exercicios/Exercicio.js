...
const nameGenreAuthor = books.map((element) => `${element.name} - ${element.genre} - ${element.author.name}`);
...
const fantasyAndSciencie = books.filter((element) => element.genre === 'Fantasia' || element.genre === 'Ficção Científica')
...
const authorOfFAS = fantasyAndSciencie.map((element) => `${element.author.name}`);
const orderAOFAS = authorOfFAS.sort()
console.log(orderAOFAS);
